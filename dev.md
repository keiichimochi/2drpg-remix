...
### 完了したタスク
- T1: Remixプロジェクト雛形作成（R1, AC-1.1/1.2/1.3） ✅
  - プロジェクト名入力→作成後に編集画面へ遷移
  - 生成される構造：maps/, tilesets/, sprites/, events/, project.json
  - 直後に自動保存が走る（トースト表示）
- T2: Prisma導入・モデル定義・マイグレーション（R1, R9） ✅
  - Project, Map, Tileset, Sprite, Locale, Flag, AIGenerationJobモデル定義
  - 初期マイグレーション作成・実行
  - Prisma Client生成

### 進行中のタスク
- T3: 画像アップロード・ストレージシステム（R3, AC-3.1/3.2） ✅
  - ✅ アップロードAPI（/api/upload）を実装
...
### 次のステップ
- T4: PixiJS基盤・レンダリングシステム（R2, R8, AC-8.3） ✅
  - ✅ @pixi/reactでApplicationコンポーネント作成
...
### 次のステップ
- T5: タイルセット管理システム（R3, AC-3.1/3.2） ✅
  - ✅ PNGタイルシート16×16自動スライス機能
  - ✅ タイルパレットUIコンポーネント
...
### 次のステップ
- T6: マップレイヤ管理UI（R2, AC-2.2） 🚧
  - ✅ レイヤー（地形/衝突/装飾 最低3種）切替・追加・順序変更
  - ✅ レイヤー可視性と不透明度制御
  - 🚧 レイヤー別タイル配置機能（次の実装フェーズで）
...
### 🚀 次の開発フェーズの推奨
- T6の完了：タイル配置機能の実装
- T7の開始：タイル描画ツール（ペイント/消しゴム/矩形/バケツ）
- T8の開始：衝突判定と領域管理
